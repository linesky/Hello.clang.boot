...
class BareboneBuilder:
    ...
    def run_kernel(self):
        ...
        self.execute_command('sudo mkdir /tmp/isos7',False)
        self.execute_command('sudo chmod 777 /tmp/isos7',False)
        self.execute_command('sudo fuse-zip "$filename" /tmp/isos7'.replace("$filename",filename),True)
        
        
       
        self.execute_command("nautilus --browser /tmp/isos7",False)
    ...
    def copy_file(self):
        ...
        if 0==0:
            self.execute_command("sudo umount /tmp/isos7",True)
